        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white rounded-lg shadow-lg overflow-hidden">
                <div class="px-6 py-4 bg-emerald-600">
                    <h1 class="text-2xl font-bold text-white">Buat Source Code Baru</h1>
                </div>

                <form action="{{ route('instructor.source-codes.store') }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
                    @csrf
                        ...
                        <div>
                            ...
                            <select name="source_code_category_id" id="source_code_category_id" required
                                ...
                                @foreach($categories as $category)
                                    <option value="{{ $category->id }}" {{ old('source_code_category_id') == $category->id ? 'selected' : '' }}>
                                        {{ $category->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="6" required
                                  class="w-full rounded-md border-gray-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500">{{ old('description') }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
